...
import { getStaffLga, getStaffType } from '@/lib/auth';
...
const OnboardersBeneficiaries = () => {
        ...
        const fetchData = async () => {
            ...
            try {
                const [beneficiariesResponse, beneficiaryTypesResponse, cadresResponse, ministriesResponse] = await Promise.all([
                    api.get('/beneficiaries/onboarder'),
    ...
    const handleOpenModal = (beneficiary?: Beneficiary) => {
        if (beneficiary) {
            ...
            setMinistryId(beneficiary.cardNumber.toString());
            setImageFile(null);
        ...
        setOpenModal(true);
        setTimeout(() => setIsModalRendered(true), 0); // Trigger after next render cycle
    };
    ...
    const handleSubmit = async () => {
        if (!firstName.trim() || !lastName.trim() || !employeeId.trim() || !beneficiaryType || !cadreId || !ministryId) {
            setError('First name, last name, Computer Number, beneficiary type, cadre, and ministry are required');
            setIsSubmitting(false);
            return;
        }
    ...
    return (
        <DashboardCard title={`List of Beneficiaries in ${staffLga || 'Unknown'} LGA`}>
            <Box display="flex" justifyContent="space-between" mb={2} gap={2} flexWrap="wrap">
                ...
                <Box display="flex" gap={2} flexWrap="wrap">
                    <TextField
                        variant="outlined"
                        label="Search by Name, Email, Phone, or Computer Number"
                        value={searchTerm}
                        ...
                        <Box sx={{ flex: 1, textAlign: { xs: 'center', sm: 'left' } }}>
                            ...
                            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                                <Typography variant="body1" color="text.secondary">
                                    <strong>Computer Number:</strong> {viewingBeneficiary?.employeeId || 'N/A'}
                                </Typography>

                                <Typography variant="body1" color="text.secondary">
                                    <strong>Card Number:</strong> {viewingBeneficiary?.cardNumber || 'N/A'}
                                </Typography>
...
export default OnboardersBeneficiaries;